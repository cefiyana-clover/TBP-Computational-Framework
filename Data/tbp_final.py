...
# ------------------------------------------------------------------------------
# 2. MANIFEST EXTRACTION (LFN TRANSLATOR MODE)
# ------------------------------------------------------------------------------
...
for idx, url in enumerate(JSON_URLS):
    ...
    # Jika file manifest (2.1 GB) sudah ada di SSD Vast.ai, wget otomatis dilewati
    if not os.path.exists(manifest_file):
        os.system(f"wget -q --show-progress --no-check-certificate {url} -O {manifest_file}")
    else:
        print(f"[STATUS] Local manifest {idx+1} found. Bypassing download.")
    
    if os.path.exists(manifest_file):
        try:
            with open(manifest_file, 'r', encoding='utf-8', errors='ignore') as f:
                data = f.read()
                # REGEX BARU: Mencari Logical File Name (LFN) internal CERN
                raw_links = re.findall(r'(/store/data/[^\s"\'\[\],]+\.root)', data)
                for link in raw_links:
                    # TRANSLATOR: Menyambungkan LFN internal ke gerbang Publik VIP
                    full_url = link.replace("/store/data", "root://eospublic.cern.ch//eos/opendata/cms")
...
if not all_target_urls:
    raise RuntimeError("[FATAL] Manifest is empty or failed to parse. Terminating process.")

all_target_urls = all_target_urls[:MAX_FILES]
print(f"[STATUS] Total root file targets queued: {len(all_target_urls)}")

# ------------------------------------------------------------------------------
# 3. ITERATIVE INGESTION AND EXTRACTION LOOP (XROOTD)
# ------------------------------------------------------------------------------
