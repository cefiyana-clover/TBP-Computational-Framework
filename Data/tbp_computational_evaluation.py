...
print("==================================================================")
print("   THE CONNECTING THREAD THEORY (TBP) COMPUTATIONAL EVALUATION    ")
print("   Ab Initio 6D Geometric Architecture Verification Script        ")
print("==================================================================\n")
...
print("[STAGE 1] FUNDAMENTAL MATRIX PARAMETERS")
...
print("[STAGE 1.2] ENERGY CEILING TEST (R_crit)")
...
print("[STAGE 1.3] KINEMATIC DISSIPATION TEST (H_lim)")
...
print("[STAGE 1.4] HEAVISIDE CUTOFF TEST (UV-FINITENESS)")
...
print("[STAGE 1.5] ELECTROWEAK SECTOR MASS DERIVATION TEST")
...
# =====================================================================
print("[STAGE 2] QCD & COSMOLOGY EVALUATION")
print("==================================================================\n")
...
# ---------------------------------------------------------------------
print("[STAGE 2.1] STRONG INTERACTION SECTOR & PROTON EQUILIBRIUM")
...
# ---------------------------------------------------------------------
print("[STAGE 2.2] COSMOLOGICAL SECTOR TEST (DIMENSIONAL DILUTION)")
...
M_total = float(OM_obs) + float(DM_obs)
...
# ---------------------------------------------------------------------
print("[STAGE 2.3] GEOMETRIC PLANCK SCALE TEST")
...
# =====================================================================
print("[STAGE 3] FERMION SPECTRUM EVALUATION")
print("==================================================================\n")
...
print("[STAGE 3.1] DYNAMIC REGIME: LEPTON HIERARCHY TEST")
L_1 = float(H_lim) / float(alpha_inv)

# Electron
LPhi_1 = 1 * e_val * float(S_metric)
delta_1 = 2 * pi_val * float(H_lim)
delta_final_1 = delta_1 - (1 * L_1)
C_1 = 12 / float(alpha_inv)
M_e = (LPhi_1 - delta_final_1) * C_1

# Muon
LPhi_2 = 2 * e_val * float(S_metric)
delta_2 = float(sp.Rational(72, 528))
delta_final_2 = delta_2 - (2 * L_1)
C_2 = float(d) + pi_val
M_mu = (LPhi_2 - delta_final_2) * C_2

# Tau
LPhi_3 = 3 * e_val * float(S_metric)
delta_3 = float(sp.Rational(576, 528))
delta_final_3 = delta_3 - (3 * L_1)
C_3 = float(sp.Rational(108, 1))
...
print("[STAGE 3.2] GEOMETRIC REGIME: QUARK DYNAMICS & CONDENSATION")
delta_QCD = float(L_QCD) * float(S_metric)
...
# Charm
M_c_bare = float(R_crit_MeV) / ((2 * float(d) + 1)**2)
M_c = M_c_bare + (4 * delta_QCD)

# Bottom
M_b_bare = float(R_crit_MeV) / (24 * float(S_metric))
...
print("[STAGE 3.3] LEAST ACTION EVALUATION (VARIATIONAL CALCULUS)")
phi = sp.Symbol('phi')
...
# =====================================================================
print("[STAGE 4] BOUNDARY & UNITARITY EVALUATION")
print("==================================================================\n")

R_crit_val = sp.Rational(1936, 9)

print("[STAGE 4.1] HIGH-ENERGY ASYMPTOTIC LIMIT TEST")
...
print("[STAGE 4.2] UNITARITY AND INFORMATION CAPACITY EVALUATION")
total_nodes = 36
dim_ext = 4
nodes_ext = dim_ext**2
...
print("[STAGE 4.3] ISOSPIN ASYMMETRY EVALUATION")
...
print("[STAGE 4.4] DIMENSIONAL CONSISTENCY TEST (GRAVITATIONAL DILUTION)")
dim_target = 4
dim_area = -2
...
# =====================================================================
print("[STAGE 5] EXTREME CONDITIONS EVALUATION")
print("==================================================================\n")

print("[STAGE 5.1] TOPOLOGICAL DEFECT ISOLATION EVALUATION")
matrix_nodes = [10] * 36
...
print("[STAGE 5.2] LORENTZ INVARIANCE BOUNDARY TEST (LIV)")
energy_low = 100
energy_crit = 216
...
print("[STAGE 5.3] MACROSCOPIC CONTINUITY TEST (GRAVITATIONAL WAVES)")
...
# =====================================================================
print("[STAGE 6] OBSERVER BIAS & NEUTRINO EVALUATION")
print("==================================================================\n")

print("[STAGE 6.1] 2-LOOP KINEMATIC BACKSCATTERING (F2)")
# F2 Operator
...
Delta_M_p_1loop = sp.Rational(25, 132) # Equals 100/528
...
print("[STAGE 6.2] LOCAL OBSERVER BIAS & METRIC COMPRESSION (ALPHA^-1)")
...
print(f"  -> TBP Ideal Geometric Alpha  : {float(alpha_inv):.6f}")
print(f"  -> Local Gravitational Bias   : ~{Sigma_Phi:.3e} (Order 10^-7)")
print(f"  -> Observer Compressed Alpha  : {alpha_obs:.6f}\n")

print("[STAGE 6.3] NEUTRINO DISSIPATION MASS DERIVATION")
R_crit_sq = float(R_crit**2)
delta_1_f = float(delta_final_1)
delta_2_f = float(delta_final_2)
delta_3_f = float(delta_final_3)
